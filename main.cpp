...
//--------------------------------------------
	...
	while (get_true())
	{
		...
		{
			ir_img_isvalid = false;
			color_img_isvalid = false;
			depth_img_isvalid = false;

			char str[256];
			sprintf(str, "%04d", count);
			
			String depth_dir = dir + "/depth_" + String(str) + ".png";
			imwrite(depth_dir, mImageDepth);

			//String color_dir = dir + "/color_" + String(str) + ".png";
			//imwrite(color_dir, cImageBGR);

			//String ir_dir = dir + "/ir_" + String(str) + ".png";
			//imwrite(ir_dir, mImageIr);
